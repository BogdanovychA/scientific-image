...
### Чорні діри
- Об'єкти, де простір-час викривлений так сильно, що ніщо (навіть світло) не може вирватися назовні — за межу *горизонту подій*. У ЗТВ існують також часово-інвертовані розв'язки — **[білі діри](bili-diry.md)** (в які ніщо не може увійти), а також топологічні мости простору-часу — [кротові нори](krotovi-nory.md) (міст Ейнштейна — Розена). Перший розв'язок (Шварцшильд) описує нерухому чорну діру; пізніше (Керр) знайдено розв'язок для *обертової* чорної діри (простір-час закручується, з'являється ергосфера). Детальніше — у [Мегасвіт](mehasvit.md).
...
- Основні підходи до узгодження:
  ...
  - **Петльова квантова гравітація та спінові мережі (Карло Ровеллі)**: простір-час дискретний — мережа квантових петель і вузлів; у реляційній фізиці реальність визначається зв'язками між подіями, а не окремими об'єктами (див. [Надсвітлові явища](nadsvitlovi-yavyshcha.md)). Роджер Пенроуз у **теорії твісторів** розглядає причинні світлові промені як первинні елементи будови простору-часу, пов'язуючи гравітаційний колапс квантових суперпозицій із виникненням теперішнього часу. За гіпотезою **ER = EPR** (програма *It from Qubit*) сам простір-час «зшивається» з квантової заплутаності. Зникають сингулярності (Великий вибух стає переходом між квантовими станами геометрії). Відкритим лишається питання, як із дискретної геометрії народжується гладкий простір-час ЗТВ, і як це перевірити експериментально.
...
- [Білі діри](bili-diry.md) — зв'язок: часово-інвертований аналог чорної діри в тому самому розв'язку рівнянь Ейнштейна, непрохідний міст Ейнштейна — Розена.
